...
- [X] T044 Docker quickstart validation — `cargo test --all` inside the project Docker container drove three iterations of fixes: (1) `--keep-intermediates=false` was invalid tectonic flag syntax, silently failing the bundle warm-up → fixed to bare `tectonic <path>` call; (2) tectonic embeds a wall-clock timestamp in PDF metadata → added `SOURCE_DATE_EPOCH=1` env var on every engine invocation for FR-009 byte-identity; (3) sandbox test assertion required `.failure()` but tectonic gracefully discards `\write18` under `--untrusted` → rewrote assertion around the falsifiable side-effect-file check, and added tectonic's first-class `--untrusted` flag to the sandbox directive for a cleaner FR-017 lever. GitHub CI on PR #22 runs the same `cargo fmt --check` + `clippy -D warnings` + `cargo test --all` matrix and is green after these fixes. The full 7-step [quickstart.md](./quickstart.md) walkthrough remains a nice-to-do manual smoke for a real user-facing rehearsal but is not blocking — CI's automated run of every integration test file covers the same code paths.
